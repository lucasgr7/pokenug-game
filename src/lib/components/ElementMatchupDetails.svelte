...
<div class="space-y-4">
	...
	>
		<div class="text-[10px] font-black uppercase tracking-[0.18em] text-[var(--text-muted)]">{$_('elementMatchup.matchupTitle')}</div>
		<div class="mt-1 flex items-center gap-2 text-lg font-black" style={`color:${ELEMENT_COLOR[element]};`}>
			...
			<span>{$_('elements.' + element)}</span>
		</div>
		<p class="mt-2 text-sm text-[var(--text-muted)]">
			{$_('elementMatchup.matchupDesc')}
		</p>
	</div>

	<div class="grid gap-3 sm:grid-cols-2">
		<section class="rounded-2xl border border-emerald-500/25 bg-emerald-500/8 p-3">
			<div class="text-[10px] font-black uppercase tracking-[0.18em] text-emerald-300">{$_('elementMatchup.strongAgainst')}</div>
				...
				{#if strengths.length > 0}
					{#each strengths as target (target)}
						<span class="rounded-full border border-emerald-400/25 bg-emerald-400/10 px-2.5 py-1 text-xs font-bold text-emerald-100">
							{ELEMENT_EMOJI[target]} {$_('elements.' + target)}
						</span>
					{/each}
				{:else}
					<span class="text-sm text-[var(--text-muted)]">{$_('elementMatchup.noStrengths')}</span>
				{/if}
			</div>
		</section>

		<section class="rounded-2xl border border-sky-500/25 bg-sky-500/8 p-3">
			<div class="text-[10px] font-black uppercase tracking-[0.18em] text-sky-300">{$_('elementMatchup.weakAgainst')}</div>
			<div class="mt-2 flex flex-wrap gap-2">
				{#if weaknesses.length > 0}
					...
					{/each}
				{:else}
					<span class="text-sm text-[var(--text-muted)]">{$_('elementMatchup.noWeaknesses')}</span>
				{/if}
			</div>
		</section>
	</div>
